<template>
  <div class="ratings">
    <div class="ratings-content">
      ...
      <Split/>
      <RatingSelect :selectType="selectType" :hasRatings="hasRatings"/>
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      PubSub.subscribe('toggleType', (msg, selectType) => {
        this.selectType = selectType
      })
      PubSub.subscribe('toggleHasRatings', (msg) => {
        this.hasRatings = !this.hasRatings
      })
    },
    computed:{
      ...mapState(['ratings','info']),
      filterRatings(){
        const {ratings,selectType, hasRatings} = this
        return ratings.filter(rating => {
          const {rateType, text} = rating
          return (selectType===2 || selectType===rateType) && (!hasRatings || text.length>0)
        })
      }
    },
    components:{
